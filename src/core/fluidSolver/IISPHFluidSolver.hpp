...
namespace FluidSolver {
    ...
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::initialize() {
...
#pragma omp parallel for
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::CalculateDensity(pIndex_t particleIndex) {
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::CalculateNonPressureAccelerationAndPredictedVelocity(
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    glm::vec2 IISPHFluidSolver<Kernel, NeighborhoodSearch>::ComputeViscosityAcceleration(pIndex_t particleIndex) {
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::ComputeSourceTerm(pIndex_t particleIndex) {
        float particleDensity = collection->get<ParticleData>(particleIndex).density;


        const glm::vec2 &particlePredictedVelocity = collection->get<IISPHParticleData>(
                particleIndex).predicted_velocity;
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::ComputeDiagonalElement(pIndex_t particleIndex) {
        float sum = 0.0f;
        const glm::vec2 &particlePosition = collection->get<MovementData>(particleIndex).position;
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::InitializePressure(pIndex_t particleIndex) {
        collection->get<ParticleData>(particleIndex).pressure = 0.0f;
    }

    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::IntegrateParticle(pIndex_t particleIndex) {
        auto type = collection->get<ParticleInfo>(particleIndex).type;
        if (type == ParticleTypeBoundary) {
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::ComputePressure() {
        size_t iteration = 0;
...
#pragma omp parallel for
            for (pIndex_t particleIndex = 0; particleIndex < collection->size(); particleIndex++) {
                ...
                const glm::vec2 &particlePressureAcceleration = collection->get<MovementData>(
                        particleIndex).acceleration;
                const glm::vec2 &particlePosition = collection->get<MovementData>(particleIndex).position;
    ...
    template<typename Kernel, typename NeighborhoodSearch>
    void IISPHFluidSolver<Kernel, NeighborhoodSearch>::adapt_collection(ParticleCollection &collection) {
        FLUID_ASSERT(!collection.is_type_present<IISPHParticleData>());
