...
FluidSolver::ZIndexGridSorter::ZIndexGridSorter(float gridCellSize,
    ...
    : GridCellSize(gridCellSize), particleCollection(particleCollection)
{
}

void FluidSolver::ZIndexGridSorter::CalculateGridCoordinates(FluidSolver::particleIndex_t particleIndex, int32_t& gridX,
                                                             int32_t& gridY)
{
                                                                     ...
                                                                     uint32_t& gridX, uint32_t& gridY)
{
    ...
    gridX = (uint32_t)(((int64_t)floor(pos.x / GridCellSize)) +
                       ((int64_t)std::numeric_limits<int32_t>::max())); // a simple cast to int would round towards
                                                                        // zero, hence cause errors
