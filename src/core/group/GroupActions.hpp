...
namespace FluidSolver::GroupActions {
     ...
     * @tparam Group of particles. A Group type has to provide the following things:
     *      GroupIterator
     *          An iterator that allows to iterate over the particles in the group.
     *          Dereferencing the iterator yields a pIndex_t particle index.
     *          The iterator must only support forward iterating and dereferencing to retrieve the
     *          particle index.
     *      GroupIterator begin();
     *      GroupIterator end();
    ...
    size_t count(const Group &group) {
