...
{
	if (open_sensor_windows.size() <= index) open_sensor_windows.resize(index + 1, false);
...
{
	if (open_sensor_windows.size() <= index)return false;
...
void FluidUi::StatisticsUi::render_particle_count_sensor(FluidSolver::Sensors::GlobalParticleCountSensor* sensor)
{
	if (ImPlot::BeginPlot("Energy")) {
		...
		ImPlot::PlotLineG("Normal", [](void* data, int x) {
			auto s = (FluidSolver::SensorData< FluidSolver::Sensors::GlobalParticleCountSensor::ParticleCountData>*)data;
