...
namespace LibFluid::Raytracer
{
        ...
        for (size_t current_depth = 0; current_depth < settings.maximum_recursion_depth; current_depth++)
        {
            ...
            // importance sampling: sample the light source
            {
                ...
                Ray shadow_ray;
                {
                    if (skybox.has_data())
                    {
                        ...
                        shadow_ray.normalized_direction =
                            skybox.sample_normalized_direction(sampler.get_uniform_sampled_pair(), shadow_ray_pdf);
                    ...
                    else
                    {
                        ...
                        shadow_ray = Distributions::CosineWeightedHemisphereDistribution::sample_ray(
                            sampler.get_uniform_sampled_pair(), current.intersection.normal_at_intersection);
                        shadow_ray.starting_point = current.intersection.point_of_intersection;
                    }
                }
                ...
                if (!accelerator.is_intersecting_with_particles(shadow_ray))
                {
                    ...
                    float cosine_term =
                        glm::dot(current.intersection.normal_at_intersection, shadow_ray.normalized_direction);
            ...
            // sample a new ray and determine its intersection
            {
                ...
                if (accelerator.is_intersecting_with_particles(new_ray, new_intersection_result))
                {
                    ...
                    float cosine_term =
                        glm::dot(current.intersection.normal_at_intersection, new_ray.normalized_direction);
    ...
    Ray FluidRaytracer3D::sample_at(const IntersectionResult &intersection)
    {
        ...
        case IntersectionResult::IntersectionResultType::RayHitBoundarySurface: {
            auto ray = Distributions::CosineWeightedHemisphereDistribution::sample_ray(
                sampler.get_uniform_sampled_pair(), intersection.normal_at_intersection);
    ...
    glm::vec3 FluidRaytracer3D::bsdf(const Ray &outgoing_radiance_ray, const IntersectionResult &intersection_result,
                                     const Ray &incoming_radiance_ray)
    {
    ...
    void FluidRaytracer3D::set_view(const glm::vec3 &position, const glm::vec3 &view_direction,
