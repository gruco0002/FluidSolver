...
namespace LibFluid::Raytracer
{
    ...
    void ParticleIntersectionAccelerator::calculate_aabb()
    {
        ...
        particle_surrounding_aabb =
            particle_surrounding_aabb.extended_in_all_directions_by(particle_size * Math::kernel_support_factor);
    }

    ParticleIntersectionAccelerator::VolumeEvaluationResult ParticleIntersectionAccelerator::
        evaluate_volume_at_position(const glm::vec3 &position)
    {
    ...
    bool ParticleIntersectionAccelerator::is_surface_intersected(
        const ParticleIntersectionAccelerator::VolumeEvaluationResult &last,
        const ParticleIntersectionAccelerator::VolumeEvaluationResult &current, IntersectionResult &result)
    {
        ...
        if (last_volume_state == VolumeEvaluationResult::VolumeState::Outside)
        {
            ...
            result.point_of_intersection =
                interpolate_between_vectors(last.position, current.position, interpolation_factor);

            if (current_volume_state == VolumeEvaluationResult::VolumeState::Fluid)
            {
                // and reached the fluid
                result.intersection_result_type =
                    IntersectionResult::IntersectionResultType::RayReachedFluidSurfaceFromOutsideTheFluid;
                result.normal_at_intersection = Math::safe_normalize(interpolate_between_vectors(
                    last.fluid_only_normal, current.fluid_only_normal, interpolation_factor));
            }
            else if (current_volume_state == VolumeEvaluationResult::VolumeState::Boundary)
            {
                ...
                result.normal_at_intersection = Math::safe_normalize(interpolate_between_vectors(
                    last.boundary_only_normal, current.boundary_only_normal, interpolation_factor));
            }
        }
        else if (last_volume_state == VolumeEvaluationResult::VolumeState::Fluid)
        {
            ...
            if (current_volume_state == VolumeEvaluationResult::VolumeState::Outside)
            {
                ...
                result.intersection_result_type =
                    IntersectionResult::IntersectionResultType::RayReachedFluidSurfaceFromInsideTheFluid;
            ...
            else if (current_volume_state == VolumeEvaluationResult::VolumeState::Boundary)
            {
                ...
                float interpolation_factor = (last.boundary_only_density - last.fluid_only_density) /
                                             (current.fluid_only_density - last.fluid_only_density -
                                              current.boundary_only_density + last.boundary_only_density);
                result.point_of_intersection =
                    interpolate_between_vectors(last.position, current.position, interpolation_factor);
                result.normal_at_intersection = Math::safe_normalize(interpolate_between_vectors(
                    last.boundary_only_normal, current.boundary_only_normal, interpolation_factor));
    ...
    ParticleIntersectionAccelerator::VolumeEvaluationResult::VolumeState ParticleIntersectionAccelerator::
        get_volume_state(const ParticleIntersectionAccelerator::VolumeEvaluationResult &result) const
    {
        float iso_surface_density = surface_density_as_fraction_of_rest_density * rest_density;
    ...
    glm::vec3 ParticleIntersectionAccelerator::interpolate_between_vectors(const glm::vec3 &a, const glm::vec3 &b,
                                                                           float factor)
    {
