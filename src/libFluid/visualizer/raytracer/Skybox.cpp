...
namespace LibFluid::Raytracer
{
    ...
    inline glm::vec3 normalized_cartesian_from_polar_direction(const glm::vec2 &polar)
    {
        ...
        return {std::sin(polar.x) * std::cos(polar.y), std::cos(polar.x), std::sin(polar.x) * std::sin(polar.y)};
    }
    ...
    void Skybox::prepare()
    {
        ...
        for (size_t i = 0; i < size; i++)
        {
            ...
            auto luminance = glm::dot(glm::vec3(radiance_value.x, radiance_value.y, radiance_value.z),
                                      glm::vec3(0.299f, 0.587f, 0.144f));
        ...
        skybox_image_distribution = Distributions::PiecewiseConstantDistribution2D(
            skybox_luminance_distribution_values, filtered_skybox.width(), filtered_skybox.height());
    }
