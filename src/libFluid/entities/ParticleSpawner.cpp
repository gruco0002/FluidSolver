...
namespace LibFluid
{
        ...
        if (time_left_over * parameters.initial_velocity >= simulation_data.particle_size)
        {
            ...
            for (float x = -width / 2.0f; x <= width / 2.0f; x += simulation_data.particle_size)
            {
                glm::vec2 pos = parameters.position + orth_direction * x +
                                (time_left_over * parameters.initial_velocity - simulation_data.particle_size) *
    ...
    size_t ParticleSpawner::get_or_add_particle()
    {
        ...
        {
            if (simulation_data.collection->get<ParticleInfo>(last_index_checked).type ==
                ParticleType::ParticleTypeInactive)
            {
    ...
    void ParticleSpawner::spawn_particle(size_t index, const glm::vec2 &position, const glm::vec2 &initial_velocity)
    {
    ...
    bool ParticleSpawner::is_position_free(const glm::vec2 &position)
    {
        ...
        float min_distance_squared =
            (simulation_data.particle_size * epsilon) * (simulation_data.particle_size * epsilon);
