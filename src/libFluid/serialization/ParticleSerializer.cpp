...
namespace esbs
{
    template <> struct EndianSafeBinaryStreamExtension<glm::vec2>
    {
    ...
    template <> struct EndianSafeBinaryStreamExtension<glm::vec3>
    {
        template <typename Stream>
...
namespace LibFluid::Serialization
{
    ...
    void ParticleSerializer::serialize(ParticleCollection &collection)
    {
    ...
    ParticleSerializer::ParticleSerializer(std::filesystem::path filepath) : filepath(std::move(filepath))
    {
    }
    ...
    void ParticleSerializer::write_component_ids(esbs::EndianSafeBinaryStream<Lz4CompressedStream> &stream,
    ...
    void ParticleSerializer::add_components_if_required(
        const ParticleSerializer::AvailableComponents &available_components, ParticleCollection &collection)
    {
        if (available_components.movement_data)
        {
            if (!collection.is_type_present<MovementData>())
    ...
    ParticleSerializer::AvailableComponents ParticleSerializer::read_available_components(
        esbs::EndianSafeBinaryStream<Lz4CompressedStream> &stream)
    {
