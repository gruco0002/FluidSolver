    ...
    class MainSerializer
    {
      public:
        struct SerializeSettings
        {
            ...
            SerializeSettings() : save_particle_data(true), particle_data_relative_filepath("particles.data")
            {
            }
            SerializeSettings(bool save_particle_data, const std::string &particle_data_relative_filepath)
                : save_particle_data(save_particle_data),
                  particle_data_relative_filepath(particle_data_relative_filepath)
            {
            }
        };
      ...
      public:
        ...
        void serialize_bundle(SimulatorVisualizerBundle &bundle,
                              const SerializeSettings &settings = SerializeSettings(),
                              SerializationContext * = nullptr);
        void serialize_simulator_only(std::shared_ptr<Simulator> simulator,
                                      const SerializeSettings &settings = SerializeSettings(),
                                      SerializationContext * = nullptr);
