...
namespace LibFluid
{
    std::tuple<float, float> DynamicCflTimestepGenerator::calculate_maximum_velocity_and_acceleration()
    {
        ...
        FLUID_ASSERT(parameters.particle_collection->is_type_present<MovementData>() ||
                     parameters.particle_collection->is_type_present<MovementData3D>());
    ...
    void DynamicCflTimestepGenerator::generate_next_timestep()
    {
        ...
        if (max_velocity > std::numeric_limits<float>::epsilon() &&
            max_acceleration > std::numeric_limits<float>::epsilon())
        {
