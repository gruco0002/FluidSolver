 ...
 * @tparam Args Variable amount of parameters which
     * can be exchanged with the delegate.
 */
    template <class... Args> class EventDelegate {
      ...
      public:
        ...
        /**
         * Subscribes a method/function to the event delegate which wants to receive the arguments specified
         * in Args.
         ...
         * @note To
         * subscribe a method of an object to the event delegate use std::bind()
         */
        uint32_t Subscribe(std::function<void(Args...)> handle);

        /**
         * Unsubscribes a method/function from the event delegate.
         * @param subscriberID The handle
         * which was returned previously at the time of subscription.
         */
        void Unsubscribe(uint32_t subscriberID);

        /**
         * Publishes Args arguments to the subscribers.
         * @param args The arguments which should be
         * published
         */
        void Fire(Args... args);
    ...
    template <class... Args> EventDelegate<Args...>::EventDelegate()
    {
        count = 0;
    }

    template <class... Args> uint32_t EventDelegate<Args...>::Subscribe(std::function<void(Args...)> handle)
    {
    ...
    template <class... Args> void EventDelegate<Args...>::Unsubscribe(uint32_t subscriberID)
    {
    ...
    template <class... Args> void EventDelegate<Args...>::Fire(Args... args)
    {
