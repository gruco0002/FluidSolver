...
namespace Engine
{
    namespace Graphics
    {
        namespace Buffer
        {
             ...
             */
            template <typename T> class IndexBuffer : public Buffer {
            ...
            template <typename T> void IndexBuffer<T>::UpdateData(std::vector<T>& data)
            {
            ...
            template <typename T> void IndexBuffer<T>::UpdateData(T* data)
            {
                Buffer::SetData((void*)data);
            }

            template <typename T> void IndexBuffer<T>::SetData(std::vector<T>& data)
            {
                if (this->GetElementCount() != data.size())
            ...
            template <typename T> void IndexBuffer<T>::SetData(T* data, size_t elementCount)
            {
            ...
            template <typename T> IndexBufferDataType IndexBuffer<T>::GetIndexBufferDataType()
            {
                return indexBufferDataType;
            }

            template <typename T> void IndexBuffer<T>::CheckAndSetType()
            {

                if (!std::numeric_limits<T>::is_integer)
                    throw EngineException("Index Buffer: Unsupported data type!");
