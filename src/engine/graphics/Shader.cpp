...
Engine::Graphics::Shader::Shader(std::vector<Engine::Graphics::Shader::ProgramPart> parts,
                                 std::vector<std::string> defines) : parts(parts), defines(defines) {
    Generate();
}
...
void Engine::Graphics::Shader::Generate() {
    ...
    if (isCompiled == GL_FALSE) {
        ...
        throw EngineException(
                ("Shader Error: Error linking the shader program parts together! " +
                 std::string(errorLog.data())).c_str());
    }
...
void Engine::Graphics::Shader::InsertDefinesIntoCode() {
    std::reverse(defines.begin(),
                 defines.end()); // reverse the defines, since they are added in reverse order => End result is adding in correct order
