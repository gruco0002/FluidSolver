	...
	const std::string& vPath, const std::string& vDefaultFileName, const std::string& vUserString)
{
	...
	auto pos = std::string(vFilters).find_first_of("\0");
	auto sub = std::string(vFilters).substr(0, pos);
	dlg_defaultExt = sub;
...
bool ImGuiFileDialog::FileDialog(const std::string& vKey)
{
	if (m_ShowDialog && dlg_key == vKey)
	{
		...
		const float sizeFactor = 0.6f;
		auto pos = ImVec2(viewport->Size.x - sizeFactor * viewport->Size.x,
			viewport->Size.y - sizeFactor * viewport->Size.y);
		pos.x *= 0.5f;
		pos.y *= 0.5f;

		auto size = ImVec2(sizeFactor * viewport->Size.x,
			sizeFactor * viewport->Size.y);

		ImGui::SetNextWindowPos(pos);
		ImGui::SetNextWindowSize(size);

		if (!ImGui::IsPopupOpen(dlg_name.c_str())) {
			ImGui::OpenPopup(dlg_name.c_str());
		}

		if (ImGui::BeginPopupModal(dlg_name.c_str(), 
			(bool*)0, ImGuiWindowFlags_NoResize)) {
			...
			ImVec2 size = ImGui::GetContentRegionMax() - ImVec2((float)dlg_optionsPaneWidth, 7.0f * lineHeight);
