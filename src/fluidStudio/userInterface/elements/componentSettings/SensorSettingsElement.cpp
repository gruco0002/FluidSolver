...
namespace FluidStudio
{
    ...
    void SensorSettingsElement::update()
    {
        ...
        {
            update_global_energy_sensor_component(
    ...
    void SensorSettingsElement::update_sensor_plane_component(std::shared_ptr<LibFluid::Sensor> tmp)
    {
        ...
        if (StyledImGuiElements::slim_tree_node("Value"))
        {
            ...
            {
                if (ImGui::Selectable("Density", sen->settings.sensor_type ==
                                                     SensorPlane::SensorPlaneType::SensorPlaneTypeDensity))
                {
                    sen->settings.sensor_type = SensorPlane::SensorPlaneType::SensorPlaneTypeDensity;
                }

                if (ImGui::Selectable("Velocity", sen->settings.sensor_type ==
                                                      SensorPlane::SensorPlaneType::SensorPlaneTypeVelocity))
                {
                    sen->settings.sensor_type = SensorPlane::SensorPlaneType::SensorPlaneTypeVelocity;
                }

                if (ImGui::Selectable("Pressure", sen->settings.sensor_type ==
                                                      SensorPlane::SensorPlaneType::SensorPlaneTypePressure))
                {
