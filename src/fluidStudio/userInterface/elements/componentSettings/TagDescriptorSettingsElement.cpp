        ...
        if (StyledImGuiElements::slim_tree_node("Edit Particles")) {
            {
                if (ImGui::BeginCombo("Type", TypeInformationProvider::particle_type_to_string(particle_type))) {
                    if (ImGui::Selectable(TypeInformationProvider::particle_type_to_string(LibFluid::ParticleTypeNormal), particle_type == LibFluid::ParticleTypeNormal)) {
                        particle_type = LibFluid::ParticleTypeNormal;
                    }
                    if (ImGui::Selectable(TypeInformationProvider::particle_type_to_string(LibFluid::ParticleTypeBoundary), particle_type == LibFluid::ParticleTypeBoundary)) {
                        particle_type = LibFluid::ParticleTypeBoundary;
                    }
                    if (ImGui::Selectable(TypeInformationProvider::particle_type_to_string(LibFluid::ParticleTypeInactive), particle_type == LibFluid::ParticleTypeInactive)) {
