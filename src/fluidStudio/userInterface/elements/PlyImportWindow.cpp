            ...
            for (int x = 0; x < particle_multiplier; x++)
            {
                for (int y = 0; y < particle_multiplier; y++)
                {
                    for (int z = 0; z < particle_multiplier; z++)
                    {
                        ...
                        pos.position = scaled_vertex * (float)particle_multiplier +
                                       glm::vec3((float)x, (float)y, (float)z) * particle_size;
        ...
        if (ImGui::Begin("Ply Import", &visible))
        {
            ...
            for (auto mapped : mapped_colors)
            {
                ...
                if (ImGui::BeginCombo("Type",
                                      TypeInformationProvider::particle_type_to_string(mapped_info.particle_type)))
                {
                    if (ImGui::Selectable(TypeInformationProvider::particle_type_to_string(
                                              LibFluid::ParticleType::ParticleTypeNormal),
                    ...
                    if (ImGui::Selectable(TypeInformationProvider::particle_type_to_string(
                                              LibFluid::ParticleType::ParticleTypeBoundary),
                    ...
                    if (ImGui::Selectable(TypeInformationProvider::particle_type_to_string(
                                              LibFluid::ParticleType::ParticleTypeInactive),
