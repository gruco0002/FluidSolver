...
namespace FluidStudio
{
        ...
        if (ImGui::Begin("Components"))
        {
            ...
            if (begin_structural_node("Sensors"))
            {
                ...
                for (size_t i = 0; i < ui_data.window().simulator_visualizer_bundle.simulator->data.sensors.size(); i++)
                {
                    ...
                    // set name
                    auto name =
                        sen->parameters.name + ": " + TypeInformationProvider::get_sensor_type_name(sen) + " Sensor";
            ...
            if (begin_structural_node("Entities"))
            {
                ...
                for (size_t i = 0; i < ui_data.window().simulator_visualizer_bundle.simulator->data.entities.size();
                     i++)
                {
                    ...
                    // set name
                    auto name =
                        "Entity " + std::to_string(i + 1) + ": " + TypeInformationProvider::get_entity_type_name(ent);
            ...
            if (begin_structural_node("Tag Descriptors"))
            {
                for (size_t i = 0;
                     ...
                     i++)
                {
                    const auto &title = ui_data.window()
                                            .simulator_visualizer_bundle.simulator->data.tag_descriptors->descriptors[i]
                                            .title;
    ...
    void ComponentPanelWindow::delete_component(const SimulationComponent &component)
    {
        if (component.kind == SimulationComponent::Kind::Sensor)
        {
            ...
            // delete the sensor
            for (size_t i = component.index;
                 i < ui_data.window().simulator_visualizer_bundle.simulator->data.sensors.size() - 1; i++)
            {
                ui_data.window().simulator_visualizer_bundle.simulator->data.sensors[i] =
                    ui_data.window().simulator_visualizer_bundle.simulator->data.sensors[i + 1];
            }
        ...
        else if (component.kind == SimulationComponent::Kind::Entity)
        {
            ...
            // delete the entity
            for (size_t i = component.index;
                 i < ui_data.window().simulator_visualizer_bundle.simulator->data.entities.size() - 1; i++)
            {
                ui_data.window().simulator_visualizer_bundle.simulator->data.entities[i] =
                    ui_data.window().simulator_visualizer_bundle.simulator->data.entities[i + 1];
            }
    ...
    void ComponentPanelWindow::update_selection_based_ui()
    {
        ...
        else if (m_selection.kind == SimulationComponent::Kind::TagDescriptor)
        {
            FLUID_ASSERT(
                m_selection.index <
                ui_data.window().simulator_visualizer_bundle.simulator->data.tag_descriptors->descriptors.size());
            auto td = ui_data.window()
                          .simulator_visualizer_bundle.simulator->data.tag_descriptors->descriptors[m_selection.index];
            ...
            {
                new_overlay_instance = std::make_shared<ParticleSelectionByTagOverlay>(
                    ui_data.window().simulator_visualizer_bundle.simulator, td.particle_tag);
            }
        }
