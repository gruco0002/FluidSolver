...
void FluidUi::UiLayer::render_component_panel()
{
    if (ImGui::Begin("Components"))
    {
        ...
        for (size_t i = 0; i < window->simulation.simulation->data.sensors.size(); i++)
        {
            auto sen = window->simulation.simulation->data.sensors[i];
        ...
        if (ImGui::BeginPopupContextWindow(0, 1, false))
        {
            if (ImGui::BeginMenu("Add Sensor"))
            {
                if (ImGui::MenuItem("Global Density", nullptr, nullptr, is_safe))
                {
                    ...
                    sen->parameters.name = "Sensor " + std::to_string(window->simulation.simulation->data.sensors.size() + 1);
                    window->simulation.simulation->data.sensors.push_back(sen);
                }
                ...
                for (auto& name : window->solver_types.names_solver)
                {
                    if (ImGui::Selectable(name.c_str(), window->current_type->name_solver == name))
                    {
                        ...
                        if (t && can_change)
                        {
                            ...
                            // set the new type
                            window->simulation.simulation->data.fluid_solver = t->create_type();
                        }
                    }
                }
                ImGui::EndCombo();
    ...
    if (window->current_type->settings_type == FluidSolverTypes::SolverSettingsTypeSESPH)
    {
        BeginSubsection("SESPH", [=]() {
            auto v = (FluidSolver::SESPHSettings*)window->current_type->get_settings(
                window->simulation.simulation->data.fluid_solver);
...
void FluidUi::UiLayer::render_timestep_component()
{
    BeginSubsection("Timestep", [=]() {
        auto ct = std::dynamic_pointer_cast<FluidSolver::ConstantTimestepGenerator>(window->simulation.simulation->data.timestep_generator);
        auto dt = std::dynamic_pointer_cast<FluidSolver::DynamicCflTimestepGenerator>(window->simulation.simulation->data.timestep_generator);
        ...
        if (ImGui::BeginCombo("Type", ct ? "Constant" : "Dynamic CFL"))
        {
            if (ImGui::Selectable("Constant", ct != nullptr))
            {
                if (ct == nullptr)
                {
                    ...
                    window->simulation.simulation->data.timestep_generator = ct;
                    window->simulation.simulation->data.notify_that_data_changed();
                }
            }
            if (ImGui::Selectable("Dynamic CFL", dt != nullptr))
            {
                if (dt == nullptr)
                {
                    ...
                    window->simulation.simulation->data.timestep_generator = dt;
                    window->simulation.simulation->data.notify_that_data_changed();
                }
            }
            ImGui::EndCombo();
...
void FluidUi::UiLayer::render_sensor_component(size_t index)
{
    ...
    FLUID_ASSERT(index < window->simulation.simulation->data.sensors.size());
    auto sen = window->simulation.simulation->data.sensors[index];
...
void FluidUi::UiLayer::render_output_component()
{
    ...
    auto& output = window->simulation.simulation->output;
...
void FluidUi::UiLayer::delete_component(const Component& component)
{
    if (component.kind == Component::Kind::Sensor)
    {
        ...
        // check for existance
        if (window->simulation.simulation->data.sensors.size() <= component.index)
        {
            return;
        }

        // delete the sensor
        for (size_t i = component.index; i < window->simulation.simulation->data.sensors.size() - 1; i++)
        {
            window->simulation.simulation->data.sensors[i] = window->simulation.simulation->data.sensors[i + 1];
        }
        window->simulation.simulation->data.sensors.pop_back();
    }
}
...
{
    ImGui::InputFloat("Gravity", &window->simulation.simulation->data.fluid_solver->parameters.gravity);
    ImGui::InputFloat("Rest Density", &window->simulation.simulation->data.fluid_solver->parameters.rest_density);
    ImGui::InputFloat("Particle Size", &window->simulation.simulation->data.fluid_solver->parameters.particle_size);
}
