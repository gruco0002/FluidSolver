...
void FluidUi::ScenariosWindow::render() {
    ...
    if (window->is_safe_to_access_simulation_data()) {
        ...
        ImGui::TextWrapped("Since you are running the simulation in asynchronous mode, you cannot load a scenario "
                           "if the simulation is running or has not stopped running!");
    }
    if (ImGui::Button("(Re)Load Scenario") && window->is_safe_to_access_simulation_data()) {
        load();
    }
}
