...
namespace FluidStudio {
    ...
    std::optional<nlohmann::json> FluidStudioRootSerializerExt::serialize(LibFluid::Serialization::SerializationContext& context) {
        nlohmann::json node;
    ...
    void FluidStudioRootSerializerExt::deserialize(std::optional<nlohmann::json> node, LibFluid::Serialization::SerializationContext& context) {
        if (!node.has_value()) {
            return;
        }
    ...
    nlohmann::json FluidStudioRootSerializerExt::serialize_editor_visualizer(LibFluid::Serialization::SerializationContext& context) {
    ...
    void FluidStudioRootSerializerExt::deserialize_editor_visualizer(const nlohmann::json& node, LibFluid::Serialization::SerializationContext& context) {
        FLUID_ASSERT(node.contains("type"));

        auto type = node["type"].get<std::string>();
        if (type == "gl-particle-renderer") {
            editor_visualizer = std::dynamic_pointer_cast<LibFluid::GLParticleRenderer>(vse_gl_particle_renderer.deserialize_visualizer(node, context));
        } else if (type == "gl-particle-renderer-3d") {
            editor_visualizer = std::dynamic_pointer_cast<LibFluid::GLParticleRenderer3D>(vse_gl_particle_renderer_3_d.deserialize_visualizer(node, context));
        }
    }
