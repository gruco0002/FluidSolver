...
Buffer::Buffer(Buffer::BufferType type, size_t elementSize, DataMode dataMode, size_t elementCount)
    : type(type), elementSize(elementSize), dataMode(dataMode), elementCount(elementCount) {
    GenerateBuffer();
}
...
void Buffer::SetData(void* data, size_t offsetInBufferInBytes, size_t sizeInBytes) {
    if (offsetInBufferInBytes + sizeInBytes > this->sizeInBytes)
        throw std::invalid_argument("Buffer Exception: Trying to write outside the buffers memory!");
...
IndexBufferDataType Buffer::GetIndexBufferDataType() {
    throw std::invalid_argument("Buffer Exception: Operation not valid for this buffer!");
}
