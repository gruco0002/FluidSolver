...
void LibFluid::GLParticleRenderer3D::create_shader_if_required()
{
    ...
    {
        LibFluid::Log::error(std::string("[GLParticleRenderer3D] Could not generate shader for 3D renderer: ") +
                             e.what());
...
void LibFluid::GLParticleRenderer3D::set_view(const glm::vec3 &position, const glm::vec3 &view_direction,
