...
TEST(CubicSplineKernelTest, SymmetricKernelTest)
{
    ...
    for (float t = 0.0f; t <= 10.0f; t += 0.001f)
    {
        ...
        EXPECT_FLOAT_EQ(kernel.GetKernelValue(glm::vec2(t, 0.2f * t)), kernel.GetKernelValue(glm::vec2(-t, -0.2f * t)));
        EXPECT_FLOAT_EQ(kernel.GetKernelValue(glm::vec2(t, 0.8f * t)), kernel.GetKernelValue(glm::vec2(-t, -0.8f * t)));
    }
}
