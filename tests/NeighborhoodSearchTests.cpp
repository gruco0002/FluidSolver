...
TYPED_TEST_P(NeighborhoodSearchTest, UniformSampledParticlesMoving)
{
    ...
    for (size_t i = 0; i < 1024; i++)
    {
        ...
        std::string currentState =
            "Failed after moving the particles " + std::to_string(i + 1) +
            " times.\nThe total distance the particles were moved is { x=" + std::to_string(totalMovement.x) +
            ", y=" + std::to_string(totalMovement.y) + " }.";
